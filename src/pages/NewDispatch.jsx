...
import { XIcon } from "../components/ui.jsx";
...
export default function NewDispatch({ onConfirm, onCancel, existingDispatches }) {
  ...
  // Separate toggle area and input area — no nested onClick conflict
  const ParcelTypeRow = ({ label, variant, checked, onToggle, count, onCount }) => {
    const pillStyle  = variant === "normal"  ? "bg-blue-50 text-blue-700"
                     : variant === "express" ? "bg-orange-50 text-orange-700"
                     :                        "bg-amber-50 text-amber-700";
    const pillLabel  = variant === "normal"  ? "Standard"
                     : variant === "express" ? "Priority"
                     :                        "Oversize";
    const borderStyle = checked ? "border-gray-300 bg-gray-50" : "border-gray-100 bg-white";

    return (
      <div className={`flex items-center rounded-xl border transition-all overflow-hidden ${borderStyle}`}>
        {/* Left tap zone — toggles checkbox */}
        <div
        ...
        {/* Right zone — input, completely separated from toggle */}
        {checked && (
          <div className="pr-3 flex-shrink-0">
            <input
              ...
              className="w-16 text-center border border-gray-200 rounded-lg py-2 text-[15px] font-mono font-semibold text-gray-900 bg-white focus:outline-none focus:border-gray-400"
            />
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-5">
        ...
        <div>
          ...
          {selectedDriver && (
            <div className="mt-2 flex items-start gap-1.5 px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#1D6FA5" strokeWidth="2" className="mt-0.5 flex-shrink-0">
                <path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0118 0z M12 10a2 2 0 100-4 2 2 0 000 4"/>
              </svg>
              {selectedDriver.type === "fixed" ? (
                ...
                <span className="text-[10px] text-amber-700 font-medium">Floating driver — suburbs assigned daily</span>
              )}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {availableVans.length === 0 ? (
            <div className="border border-amber-200 bg-amber-50 rounded-xl px-3 py-3 text-[12px] text-amber-700">
              All active vans are already dispatched today.
            </div>
          ) : (
            <select
              value={vanPlate}
            ...
            >
              ...
              {availableVans.map(v => (
                <option key={v.id} value={v.plate}>{v.plate} — {v.make} ({v.year})</option>
              ))}
            </select>
          )}
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-xl border border-gray-100">
          ...
          <span className="text-[22px] font-bold text-gray-900 font-mono">{total}</span>
        </div>

        {error && (
          <div className="text-[12px] text-ap-red font-medium text-center">{error}</div>
        )}
      </div>
